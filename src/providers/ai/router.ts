...
const DEFAULT_ORDER: AIProviderId[] = ["groq", "cerebras", "openrouter"];
...
export function getAIProviderOrder(): AIProviderId[] {
  const requested = (process.env.AI_PROVIDER_ORDER ?? "").split(",").map((item) => item.trim()).filter((item): item is AIProviderId => DEFAULT_ORDER.includes(item as AIProviderId));
...
export async function generateWithFallback(request: AIRequest, options?: AIRouterOptions): Promise<AIRouterResult> {
  ...
  for (const providerId of order) {
    ...
    try {
      ...
      return { ...result, metadata: { provider: result.provider, model: result.model, modelSource: result.modelSource, modelRole: result.modelRole, fallbackUsed: attempts > 1, attempts, failures: failureDetails } };
    } catch (error) {
  ...
  throw Object.assign(new AIProviderError(failures.length ? "All configured AI providers were unavailable." : "No AI provider is configured.", { retryable: false }), { failureDetails });
}