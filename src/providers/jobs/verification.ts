...
export async function verifyEmployer(job: NormalizedJob): Promise<JobVerification> {
  ...
  if (!employerUrl || !employerHost || isAts(employerUrl)) return { status: "unverified", score: 0, reasons: ["The job was discovered, but Vantage does not have a verified public employer website to inspect."], evidence: [] };

  const candidates = Array.from(new Set([employerUrl, job.sourceUrl, job.applyUrl].filter(Boolean) as string[])).filter((url) => { const h = hostname(url); return Boolean(h && (sameHost(url, employerUrl) || isAts(url))); });
  ...
  let bestScore = 35; let publicPageFound = false; let companyConfirmed = false; let roleConfirmed = false; let careersConfirmed = false; let agencySignal = false;
  ...
  if (bestScore >= 70 && companyConfirmed && (roleConfirmed || careersConfirmed) && !agencySignal) return { status: "direct_employer_verified", score: bestScore, reasons: ["Employer identity and job context were corroborated from public employer-domain evidence.", ...reasons], evidence: Array.from(new Map(evidence.map((item) => [`${item.url}|${item.reason}`, item])).values()).slice(0, 12), companyDomain: employerHost };
