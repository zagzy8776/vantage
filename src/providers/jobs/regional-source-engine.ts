...
function fromJobPosting(item: Record<string, unknown>, provider: RegionalJobProvider, sourceName: string, sourceUrl: string): NormalizedJob | undefined {
  ...
  if (!title || !companyName) return undefined;
  const employerWebsite = url(text(org?.url) ?? text(org?.sameAs), sourceUrl);
  const applicationUrl = url(text(item.url), sourceUrl);
  const location = locationValue(item.jobLocation);
  const countryRaw = Array.isArray(item.jobLocation) ? undefined : text((item.jobLocation as any)?.address?.addressCountry);
  const postedAt = text(item.datePosted);
  return {
    id: stableId(provider, applicationUrl ?? sourceUrl, title, companyName),
    provider, title, companyName, companyWebsite: employerWebsite, companyDomain: host(employerWebsite),
    description: strip(text(item.description) ?? ""), location,
    countryCode: countryRaw?.length === 2 ? countryRaw.toUpperCase() : undefined,
    employmentType: text(item.employmentType), remote: /telecommute|remote/i.test(JSON.stringify(item.jobLocationType ?? item.jobLocation ?? "")),
    postedAt, applyUrl: applicationUrl, sourceUrl, sourceName,
    requirements: [], verificationStatus: "unverified",
    verificationReasons: [`Discovered from ${sourceName}. Publisher provenance is retained; employer verification is performed separately.`],
    verificationEvidence: [{ url: sourceUrl, reason: `Public ${sourceName} listing/feed evidence.` }],
  };
}
...
function feedItems(xml: string, provider: RegionalJobProvider, sourceName: string, feedUrl: string, query: JobSearchQuery) {
  ...
  for (const match of xml.matchAll(/<(?:item|entry)\\b[\\s\\S]*?<\\/(?:item|entry)>/gi)) {
    ...
    const link = url(item.match(/<link[^>]*href=["']([^"']+)["']/i)?.[1] ?? item.match(/<link[^>]*>([^<]+)<\\/link>/i)?.[1], feedUrl);
    if (!title || !link || !titleMatches(title, query.title)) continue;
    const description = strip(item.match(/<(?:description|summary|content:encoded)[^>]*>([\\s\\S]*?)<\\/(?:description|summary|content:encoded)>/i)?.[1] ?? "");
    const rawDate = strip(item.match(/<(?:pubDate|published|updated)[^>]*>([\\s\\S]*?)<\\/(?:pubDate|published|updated)>/i)?.[1] ?? "");
    const creator = strip(item.match(/<(?:dc:creator|author)[^>]*>([\\s\\S]*?)<\\/(?:dc:creator|author)>/i)?.[1] ?? "");
    const parsedDate = rawDate ? new Date(rawDate) : undefined;
    jobs.push({ id: stableId(provider, link, title, creator || "unknown"), provider, title, companyName: creator || "Unknown employer", description, sourceUrl: link, sourceName: `${sourceName} feed`, postedAt: parsedDate && !Number.isNaN(parsedDate.getTime()) ? parsedDate.toISOString() : undefined, requirements: [], verificationStatus: "unverified", verificationReasons: [`Collected from ${sourceName} public feed. Employer identity and application path remain independently unverified.`], verificationEvidence: [{ url: feedUrl, reason: `Public ${sourceName} feed evidence.` }] });
    if (jobs.length >= (query.limit ?? 50)) break;
  }
  return jobs;
}

async function fetchText(target: string) {
  if (!(await isAllowedByRobots(target))) throw new Error("ROBOTS_DISALLOWED");
  const response = await fetch(target, { cache: "no-store", redirect: "follow", headers: { Accept: "text/html,application/xhtml+xml,application/xml,application/rss+xml,text/xml;q=0.9,*/*;q=0.5", "User-Agent": "VantageJobsBot/1.0" }, signal: AbortSignal.timeout(TIMEOUT_MS) });
  if (!response.ok) throw new Error(`HTTP_${response.status}`);
  return { body: await response.text(), url: response.url || target, contentType: response.headers.get("content-type") ?? "" };
}

function discoverFeedUrls(html: string, pageUrl: string) {
  const feeds = new Set<string>();
  for (const match of html.matchAll(/<link[^>]+(?:type=["'](?:application\\/(?:rss\\+xml|atom\\+xml)|text\\/xml)["'][^>]+href=["']([^"']+)["']|href=["']([^"']+)["'][^>]+type=["'](?:application\\/(?:rss\\+xml|atom\\+xml)|text\\/xml)["'])[^>]*>/gi)) {
    const found = url(match[1] ?? match[2], pageUrl);
    if (found && sameOrigin(found, pageUrl)) feeds.add(found);
  }
  for (const match of html.matchAll(/<a[^>]+href=["']([^"']+)["'][^>]*>/gi)) {
    const found = url(match[1], pageUrl);
    if (found && sameOrigin(found, pageUrl) && /(?:rss|atom|feed|xml)(?:$|[?#/])/i.test(found)) feeds.add(found);
  }
...
function candidateLinks(html: string, base: string, query: JobSearchQuery) {
  ...
  for (const match of html.matchAll(/<a\\b[^>]*href=["']([^"']+)["'][^>]*>([\\s\\S]*?)<\\/a>/gi)) {
    const found = url(match[1], base); const label = strip(match[2] ?? ""); if (!found || !label || !titleMatches(label, query.title)) continue;
    if (!sameOrigin(found, base)) continue;
    if (!/(job|career|vacanc|opening|opportunit|position|role)/i.test(`${label} ${found}`)) continue;
    if (/(login|signup|register|privacy|terms|contact|about|social)/i.test(found)) continue;
    links.add(found); if (links.size >= MAX_DETAIL) break;
  }
  return [...links];
}

function dedupe(jobs: NormalizedJob[], limit: number) {
  ...
  for (const job of jobs) {
    ...
    map.set(key, {
      ...
      verificationEvidence: Array.from(new Map([...(current.verificationEvidence ?? []), ...(job.verificationEvidence ?? [])].map((e) => [e.url, e])).values()).slice(0, 12),
    });
  }
...
export async function crawlRegionalSource(provider: RegionalJobProvider, query: JobSearchQuery): Promise<RegionalEngineResult> {
  const source = sourceDefinition(provider);
  if (!source) return { provider, status: "unavailable", jobs: [], errorMessage: "Regional source is not registered." };
  ...
  for (const seed of feedSeeds) {
    try {
      const fetched = await fetchText(seed);
      const discovered = /html/i.test(fetched.contentType) || /<(?:html|body|head)\\b/i.test(fetched.body) ? discoverFeedUrls(fetched.body, fetched.url) : [];
      const candidates = /xml|rss|atom/i.test(fetched.contentType) ? [fetched.url] : discovered;
      for (const feed of candidates.slice(0, MAX_FEEDS)) {
        try { const body = feed === fetched.url ? fetched.body : (await fetchText(feed)).body; collected.push(...feedItems(body, provider, source.name, feed, query)); } catch {}
      }
  ...
  if (source.acquisition.includes("public_web") && (source.requiresWebCrawlOptIn === false || /^(1|true|yes)$/i.test(process.env.VANTAGE_ENABLE_REGIONAL_WEB_CRAWL?.trim() ?? ""))) {
    for (const seed of source.seedUrls) {
      try {
        ...
        for (const item of jsonLd(page.body)) {
          const job = fromJobPosting(item, provider, source.name, page.url);
          if (job && titleMatches(job.title, query.title)) collected.push(job);
        }
        const links = candidateLinks(page.body, page.url, query);
        const details = await Promise.all(links.map(async (link) => { try { return await fetchText(link); } catch { return null; } }));
        for (let i = 0; i < details.length; i += 1) {
          const detail = details[i]; if (!detail) continue;
          for (const item of jsonLd(detail.body)) {
            const job = fromJobPosting(item, provider, source.name, detail.url);
            if (job && titleMatches(job.title, query.title)) collected.push(job);
          }
        }
      } catch {}
  ...
  const jobs = dedupe(collected, limit);
  return { provider, status: jobs.length ? "success" : "zero-results", jobs, totalCount: jobs.length, hasMore: false, acquisition: source.acquisition, webCrawlEnabled: /^(1|true|yes)$/i.test(process.env.VANTAGE_ENABLE_REGIONAL_WEB_CRAWL?.trim() ?? "") };
}
