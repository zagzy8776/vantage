...
async function adzuna(query: JobSearchQuery): Promise<JobProviderResult> {
  ...
  try {
    ...
    return {
      ...
      jobs: rows.flatMap((row: any) => {
        ...
        return [normalize({
          ...
          city: text(row.location?.area?.at(-1)),
          ...
          applyUrl: cleanUrl(row.redirect_url),
          sourceUrl: cleanUrl(row.redirect_url),
          sourceName: "Adzuna",
        })];
      }),
    };
    ...
    return {
      provider: "adzuna",
      status: error instanceof Error && error.message === "RATE_LIMITED" ? "rate-limited" : "failed",
      jobs: [],
      errorMessage: error instanceof Error ? error.message : "Adzuna request failed.",
    };
  }
}
...
async function jsearch(query: JobSearchQuery): Promise<JobProviderResult> {
  ...
  url.searchParams.set("country", (query.countryCode ?? "us").toLowerCase());
  url.searchParams.set("language", "en");
  if (query.remote) url.searchParams.set("work_from_home", "true");
  if (query.postedWithinDays) {
    url.searchParams.set(
      "date_posted",
      query.postedWithinDays <= 1 ? "today" : query.postedWithinDays <= 3 ? "3days" : query.postedWithinDays <= 7 ? "week" : "month",
    );
  }

  try {
    const body = await requestJson(url.toString(), { headers: { "x-api-key": key } });
    ...
    return {
      ...
      jobs: rows.flatMap((row: any) => {
        ...
        return [normalize({
          ...
          title,
          companyName,
          companyDomain: cleanUrl(row.employer_website)?.replace(/^https?:\/\/(?:www\.)?/i, "").replace(/\/$/, "") || undefined,
          ...
          salaryMin: typeof row.job_min_salary === "number" ? row.job_min_salary : undefined,
          salaryMax: typeof row.job_max_salary === "number" ? row.job_max_salary : undefined,
          salaryCurrency: text(row.job_salary_currency),
          postedAt: text(row.job_posted_at_datetime_utc),
          applyUrl: cleanUrl(row.job_apply_link),
          sourceUrl: cleanUrl(row.job_apply_link),
          sourceName: "JSearch",
        })];
      }),
    };
    ...
    return {
      ...
      status: error instanceof Error && error.message === "RATE_LIMITED" ? "rate-limited" : "failed",
      jobs: [],
      errorMessage: error instanceof Error ? error.message : "JSearch request failed.",
    };
  }
}
...
async function jobspipe(query: JobSearchQuery): Promise<JobProviderResult> {
  ...
  if (query.remote !== undefined) payload.remote = query.remote;
  ...
  try {
    ...
    return {
      ...
      status: rows.length ? "success" : "zero-results",
      jobs: rows.flatMap((row: any) => {
        const title = text(row.title ?? row.job_title);
        ...
        return [normalize({
          ...
          remote: row.remote === true ? true : undefined,
          employmentType: text(row.employment_type),
          postedAt: text(row.posted_at),
          lastSeenAt: text(row.last_seen_at),
          applyUrl: cleanUrl(row.apply_url),
          sourceUrl: cleanUrl(row.source_url ?? row.apply_url),
    ...
    return {
      ...
      status: error instanceof Error && error.message === "RATE_LIMITED" ? "rate-limited" : "failed",
      jobs: [],
      errorMessage: error instanceof Error ? error.message : "JobsPipe request failed.",
    };
  }
}
...
async function hirebase(query: JobSearchQuery): Promise<JobProviderResult> {
  ...
  try {
    ...
    return {
      ...
      jobs: rows.flatMap((row: any) => {
        ...
        return [normalize({
          ...
          companyName,
          description: text(row.description),
          location: [location?.city, location?.region, location?.country].filter(Boolean).join(", ") || undefined,
          city: text(location?.city),
          employmentType: text(row.job_type),
          remote: /remote/i.test(text(row.location_type) ?? "") ? true : undefined,
          applyUrl: cleanUrl(row.application_link),
          sourceUrl: cleanUrl(row.application_link),
          sourceName: "Hirebase",
        })];
      }),
    };
    ...
    return {
      ...
      status: error instanceof Error && error.message === "RATE_LIMITED" ? "rate-limited" : "failed",
      jobs: [],
      errorMessage: error instanceof Error ? error.message : "Hirebase request failed.",
    };
  }
}
...
async function theirStack(query: JobSearchQuery): Promise<JobProviderResult> {
  ...
  try {
    const body = await requestJson("https://api.theirstack.com/v1/jobs/search", {
      method: "POST",
      headers: { Authorization: `Bearer ${key}`, "Content-Type": "application/json" },
      body: JSON.stringify(payload),
    });
    ...
    return {
      ...
      jobs: rows.flatMap((row: any) => {
        ...
        const companyName = text(row.company);
        const id = row.job_id != null ? String(row.job_id) : undefined;
        if (!title || !companyName || !id) return [];
        return [normalize({
          id: `theirstack:${id}`,
          provider: "theirstack",
          ...
          companyName,
          description: text(row.description),
          location: Array.isArray(row.cities) ? row.cities.join(", ") : text(row.location),
          countryCode: text(row.job_country_code),
          employmentType: Array.isArray(row.employment_statuses) ? row.employment_statuses.join(", ") : text(row.employment_status),
          remote: row.remote === true ? true : undefined,
          salaryMin: typeof row.min_annual_salary_usd === "number" ? row.min_annual_salary_usd : undefined,
          salaryMax: typeof row.max_annual_salary_usd === "number" ? row.max_annual_salary_usd : undefined,
          salaryCurrency: "USD",
          postedAt: text(row.date_posted ?? row.posted_at),
          applyUrl: cleanUrl(row.final_url ?? row.url),
          sourceUrl: cleanUrl(row.final_url ?? row.url),
    ...
    return {
      ...
      status: error instanceof Error && error.message === "RATE_LIMITED" ? "rate-limited" : "failed",
      jobs: [],
      errorMessage: error instanceof Error ? error.message : "TheirStack request failed.",
    };
  }
}
...
async function verifyTopJobs(jobs: NormalizedJob[]) {
  const candidateIndexes = jobs
    .map((job, index) => ({ job, index }))
    .filter(({ job }) => job.verificationStatus !== "rejected")
    .slice(0, 12)
    .map(({ index }) => index);
  const verified = [...jobs];
      ...
      try {
        ...
        return {
          ...
          job: {
            ...
            verificationReasons: [...job.verificationReasons, "Employer verification could not be completed for this listing."],
          },
        };
      }
    }));
...
export async function runJobDiscovery(query: JobSearchQuery, selected?: JobProvider[]) {
  ...
  return {
    ...
    verification: {
      attempted: Math.min(discovered.length, 12),
