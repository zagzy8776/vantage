  ...
  for (const job of jobs) {
    ...
    await db.execute(sql`
      INSERT INTO jobs (
        ...
        ${job.salaryMin ?? null}, ${job.salaryMax ?? null}, ${job.salaryCurrency ?? null}, ${postedAt}, ${lastSeenAt}, ${job.applyUrl ?? null}, ${job.sourceUrl ?? null}, ${job.sourceName ?? null}, ${requirements}::jsonb,
      ...
      ON CONFLICT (provider, provider_job_id, owner_id) DO UPDATE SET
        ...
        salary_currency = COALESCE(EXCLUDED.salary_currency, jobs.salary_currency), posted_at = COALESCE(EXCLUDED.posted_at, jobs.posted_at), last_seen_at = EXCLUDED.last_seen_at,
        apply_url = COALESCE(EXCLUDED.apply_url, jobs.apply_url), source_url = COALESCE(EXCLUDED.source_url, jobs.source_url), source_name = COALESCE(EXCLUDED.source_name, jobs.source_name),
        requirements = EXCLUDED.requirements, verification_status = EXCLUDED.verification_status, verification_score = EXCLUDED.verification_score,
        verification_reasons = EXCLUDED.verification_reasons, stale = false, updated_at = EXCLUDED.updated_at
    `);
    await db.execute(sql`
      INSERT INTO job_verification_events (id, job_id, owner_id, status, score, evidence, reasons, observed_at)
      VALUES (${crypto.randomUUID()}, ${job.id}, ${auth.userId}, ${job.verificationStatus}, ${score}, ${evidence}::jsonb, ${reasons}::jsonb, ${now})
    `);
    persisted += 1;
  }
  return persisted;
