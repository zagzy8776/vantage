...
export class ExaEvidenceSearchProvider implements EvidenceSearchProvider {
  ...
  async search(query: EvidenceSearchQuery): Promise<EvidenceSearchResult> {
    ...
    const companyResearch = query.category !== "employment" && /\b(company|business|employer|official website|contact)\b/i.test(searchQuery);
    const startedAt = Date.now();
    try {
      const response = await fetch("https://api.exa.ai/search", {
        method: "POST",
        headers: { "x-api-key": apiKey, "Content-Type": "application/json" },
        body: JSON.stringify({ query: searchQuery, type: "fast", ...(companyResearch ? { category: "company" } : {}), userLocation: query.country, numResults: Math.min(Math.max(query.limit, 1), 10), contents: { highlights: { maxCharacters: 800 } } }),
        cache: "no-store",
      ...
      const results: EvidenceSearchResultItem[] = payload.results.filter((item) => typeof item.title === "string" && typeof item.url === "string").map((item, index) => ({ title: item.title!, url: item.url!, snippet: item.highlights?.join(" ") || item.text, rank: index + 1, publishedAt: item.publishedDate, metadata: { author: item.author, query: searchQuery } }));
