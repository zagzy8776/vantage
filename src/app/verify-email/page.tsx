...
/** "zach@corp.com" -> "z***@corp.com" (display only) */
...
const CODE_LENGTH = 6;
/** Verification codes expire after 10 minutes - mirrors the server constant. */
const INITIAL_SECONDS = 10 * 60;
/** Resend cooldown - mirrors RESEND_COOLDOWN_MS on the server. */
  ...
  const submitCode = useCallback(
    async (code: string) => {
      ...
      try {
        ...
        // Session cookie is set by the server - navigate so the app shell
        // re-mounts with authenticated state.
          ...
          <div className="flex gap-2 justify-center mb-4" onPaste={handlePaste}>
            {digits.map((digit, index) => (
              <input
                ...
                className="w-10 h-12 sm:w-11 sm:h-13 text-center text-lg font-mono font-bold text-foreground bg-surface-2 border border-border rounded-md focus:outline-none focus:border-accent focus:ring-1 focus:ring-accent/40 disabled:opacity-50"
              />
            ))}
          </div>

          {secondsLeft > 0 ? (
            <p className="text-[11px] text-subtle text-center mb-4 font-mono">
              Code expires in{" "}
            ...
            <p className="text-xs text-danger text-center mb-4" role="alert">
              This code has expired. Request a new one below.
            </p>
          )}

          {formError && (
            <p role="alert" className="text-xs text-danger border border-danger/30 bg-danger/5 rounded-md px-3 py-2 mb-4">
              {formError}
            </p>
          )}
