...
export default function DiscoverPage() {
  ...
  const persistSnapshot = useCallback((runId: string, leads: Lead[]) => {
    ...
    try {
      window.sessionStorage.setItem(DISCOVER_SNAPSHOT_KEY, JSON.stringify({ runId, results: leads, savedAt: Date.now() } satisfies DiscoverSnapshot));
    } catch {}
  }, []);
  ...
  const clearRememberedRun = useCallback((clearSnapshot = true) => {
    ...
    try {
      ...
      if (url.searchParams.has("runId")) { url.searchParams.delete("runId"); window.history.replaceState(window.history.state, "", url.toString()); }
    } catch {}
  }, []);
  ...
  const loadRun = useCallback(async (runId: string, options: { poll?: boolean } = {}) => {
    ...
    do {
      ...
      if (response.status === 404) { setSelectedRunId(runId); setStillRunningNotice(false); setWorkflowStage(null); return null; }
      ...
      if (liveResults.length > 0) { setResults(liveResults); persistSnapshot(runId, liveResults); setStillRunningNotice(!TERMINAL.includes(current.status)); }
  ...
  const loadHistory = useCallback(async () => {
    ...
    try {
      ...
      const payload = await response.json().catch(() => null);
      if (!response.ok) { if (!restored) throw new Error(payload?.error ?? "Discovery history is unavailable."); return; }
  ...
  const handleSearch = async (filters: DiscoverFilters) => {
    ...
    try {
      const response = await fetch("/api/discover", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ ...filters, limit: filters.maxResults }) });
  ...
  const handleSelectRun = async (runId: string) => {
    ...
    const run = await loadRun(runId, { poll: false }).catch((err) => { setError(err instanceof Error ? err.message : "Could not load this scan."); return null; });
    if (run && !TERMINAL.includes(run.status)) {
  ...
  const handleAnalyzeSelected = async () => {
    ...
    try {
      const response = await fetch("/api/websites/analyze/batch", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ businessIds: selectedLeads.map((lead) => lead.business.id) }) });
      const payload = await response.json().catch(() => null);
      ...
      <section className="space-y-2">
        <div className="flex items-start justify-between gap-4 flex-wrap">
          <div>
            ...
            <h1 className="text-2xl sm:text-3xl font-extrabold tracking-tight text-foreground">Discover leads</h1>
            <p className="text-sm text-subtle mt-1 max-w-xl">Pick a category and place. Every real scan is saved so you can reopen it later.</p>
          </div>
        ...
        <div className="xl:col-span-3 space-y-4">
          <section className="border border-border rounded-xl bg-surface/90 p-4 sm:p-5 shadow-card">
            <div className="mb-3">
              ...
              <p className="text-xs text-subtle mt-1">Previous searches stay here. A new location starts a new scan.</p>
            </div>
            {historyLoading ? <LoadingState message="Loading saved scans..." rows={2} /> : runs.length === 0 ? <EmptyState title="No scans yet" description="Run your first search and VANTAGE will save the complete scan for you." /> : (
              <div className="space-y-2 max-h-64 overflow-auto">
                {runs.map((run) => {
                  ...
                  return (
                    <button key={run.id} type="button" onClick={() => void handleSelectRun(run.id)} className={`w-full text-left border rounded-xl p-3 transition-colors ${active ? "border-accent bg-accent/5" : "border-border hover:border-accent/40"}`}>
          ...
          {isLoading ? <div className="space-y-3"><LoadingState message={workflowStage ?? "Starting search..."} rows={5} /></div>
            : error ? <div className="border border-danger/40 bg-danger/5 rounded-xl p-4 text-sm">{error}</div>
            : results.length === 0 ? <EmptyState title={stillRunningNotice ? "Research is still running" : "Select a scan to view results"} description={stillRunningNotice ? "VANTAGE is continuing the saved scan. You can leave this page and return to it from Your scans." : "Choose one of your saved scans above, or run a new search."} />
            : (
              <section className="space-y-3">
                ...
                <div className="grid gap-3">
                  {results.map((lead) => (
                    <LeadCard key={lead.id} lead={lead} selected={selectedIds.includes(lead.id)} onSelect={(selected) => setSelectedIds((previous) => selected ? [...previous, lead.id] : previous.filter((id) => id !== lead.id))} />
                  ))}
                </div>
              </section>
            )}
          {message && <div className="border border-info/40 bg-info/5 rounded-xl p-3 text-xs text-subtle">{message}</div>}
        </div>
      </div>
    </div>
  );
