...
import { login, resolveSafeRedirect } from "@/lib/auth-client";
...
function LoginForm() {
  ...
  const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      // Full navigation forces every authenticated shell and data query to
      // resolve the newly-issued session instead of retaining the previous
      // account's client state. This also makes account switching reliable.
      ...
      <div className="w-full max-w-sm">
        ...
        <div className="border border-border rounded-lg bg-surface p-6 shadow-sm">
          ...
          <p className="text-xs text-subtle mb-5">
            Sign in with any VANTAGE account. Signing in replaces the current browser session.
          </p>
          <Suspense
            fallback={
