...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (billable) {
      ...
      if (!reservation.ok) {
        return NextResponse.json({ error: "Research limit reached.", code: "RESEARCH_CREDITS_EXHAUSTED", credits: { remaining: reservation.remaining, limit: reservation.limit } }, { status: 402 });
      }
    }
