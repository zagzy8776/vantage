...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (offset >= ids.length) return NextResponse.json({ jobs: [], processed: 0, nextOffset: ids.length, done: true, aiAnalyzed: 0, evidenceReady: 0, applicationPaths: 0, employerSources: 0, publicContacts: 0 });
    ...
    return NextResponse.json({ jobs: results, processed: batchIds.length, nextOffset, done: nextOffset >= ids.length, aiAnalyzed, evidenceReady, applicationPaths, employerSources, publicContacts, requested: batchIds.length, found: ownedJobs.length }, { status: 200, headers: { "Cache-Control": "no-store, no-cache, must-revalidate" } });
  } catch (error) {
