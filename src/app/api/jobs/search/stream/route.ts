  ...
  const stream = new ReadableStream<Uint8Array>({ async start(controller) {
    ...
    try {
      ...
      await Promise.all(providers.map(async (provider) => {
        ...
        try { const discovery = await runJobDiscovery(query, [provider], { verify: false }); const summary = discovery.providers.find((item) => item.provider === provider); const providerJobs = discovery.jobs.filter((job) => job.provider === provider); const providerPage = discovery.pagination[provider]; result = { provider, status: summary?.status ?? (providerJobs.length ? "success" : "zero-results"), jobs: providerJobs, totalCount: providerPage?.totalCount ?? undefined, errorMessage: summary?.errorMessage ?? undefined, hasMore: Boolean(providerPage?.hasMore), nextCursor: providerPage?.nextCursor ?? undefined }; } catch (error) { result = { provider, status: "failed", jobs: [], errorMessage: error instanceof Error ? error.message : "Provider request failed." }; }
        results.set(provider, result); const incoming = withIntelligence(result.jobs); cumulative = mergeJobs(cumulative, incoming); state.count = result.jobs.length; state.totalCount = result.totalCount; state.nextCursor = result.nextCursor; state.hasMore = result.hasMore; state.errorMessage = result.errorMessage;
        ...
        if (result.jobs.length && result.status === "success") {
          state.status = "researching"; write({ type: "status", message: `Vantage found ${result.jobs.length} ${provider} listings and is resolving employer sources, application paths, and public contacts.`, providers: states });
          try {
            ...
            const researched = await researchJobs(priorityJobs, priorityJobs.length);
            const resolved = await resolveApplicationAndContactsBatch(researched, 2);
            const enriched = withIntelligence(resolved); state.researched = enriched.length; cumulative = mergeJobs(cumulative, enriched); if (enriched.length) await persistJobs(enriched, auth);
            write({ type: "provider", provider, result: { ...result, jobs: enriched }, jobs: cumulative, providers: states });
          } catch (error) { state.errorMessage = state.errorMessage ?? (error instanceof Error ? error.message : "Priority source research failed."); write({ type: "status", message: `Vantage is continuing deeper employer research for ${provider}.`, providers: states }); }
        }
        state.status = result.status === "failed" || result.status === "rate-limited" ? "failed" : "complete"; write({ type: "status", message: `${provider} is complete. Vantage is keeping deeper employer research running in the background.`, providers: states });
      }));
