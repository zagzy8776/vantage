...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    try { persistedCount = await persistJobs(result.jobs, auth); } catch (error) { persistenceFailed = true; console.error(JSON.stringify({ diagnostic: "jobs_persistence_failed", message: error instanceof Error ? error.message : String(error) })); }
    ...
    const providerAdvertisedTotals = result.providers.reduce((sum, provider) => sum + (provider.totalCount ?? 0), 0);
    ...
    const employerContacts = result.jobs.filter((job) => Boolean(job.companyPhone || job.companyEmail)).length;
    ...
    return NextResponse.json({
      ...
      verification: { checked: result.jobs.length, directEmployerVerified: directEmployerVerifiedCount, needsVerification: needsVerificationCount, remainingUnverified: result.jobs.filter((job) => job.verificationStatus === "unverified").length },
      intelligence: { attempted: intelligence.attempted, analyzed: intelligence.analyzed, evidenceReady: intelligence.evidenceReady },
      ...
      diagnostics: {
        ...
        rawProviderHits,
        dedupedReturned: result.jobs.length,
        providerAdvertisedTotals,
        returned: result.jobs.length,
        applicationPaths,
        employerSources,
        employerContacts,
        evidenceReady: intelligence.evidenceReady,
        aiAnalyzed: intelligence.analyzed,
      ...
      policy: { directEmployerVerification: "public-source-evidence-required", fabricatedData: false, jobRequirements: "source-grounded-ai-or-deterministic-evidence", employerContacts: "public-employer-domain-evidence-required", pagination: "provider-backed" },
...
export async function GET(request: NextRequest) {
  ...
  if (auth instanceof NextResponse) return auth;
  try {
    const params = new URL(request.url).searchParams; const limitRaw = Number(params.get("limit") ?? 30); const pageRaw = Number(params.get("page") ?? 1);
    const limit = Number.isFinite(limitRaw) ? Math.max(1, Math.min(Math.floor(limitRaw), 100)) : 30; const page = Number.isFinite(pageRaw) ? Math.max(1, Math.floor(pageRaw)) : 1;
