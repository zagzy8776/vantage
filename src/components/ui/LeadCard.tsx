...
export function LeadCard({ lead, selected = false, onSelectedChange, onAnalyzeWebsite }: LeadCardProps) {
  ...
  const sourceLabel = lead.business.sources && lead.business.sources.length > 0
    ? lead.business.sources.map((source) => source.charAt(0).toUpperCase() + source.slice(1)).join(" + ")
    : lead.business.source.charAt(0).toUpperCase() + lead.business.source.slice(1);

  return (
    <Link href={`/leads/${lead.id}`} className="group block">
      <div
        className={cn(
          "bg-surface border border-border rounded-lg p-3 sm:p-4 flex gap-3 transition-all hover:border-accent/30 hover:shadow-md cursor-pointer"
        )}
      >
        <div className="w-8 h-8 rounded flex-shrink-0 bg-surface-2 border border-border-strong/40 text-accent font-mono font-bold text-xs flex items-center justify-center">
        ...
        <div className="flex-1 min-w-0 space-y-1.5">
          ...
          {lead.websiteAnalysis && (
            <p className="text-[10px] text-subtle font-mono truncate">
              Website health: {lead.websiteAnalysis.technicalHealthScore ?? "—"} / 100
            </p>
          )}
          ...
          <p className="text-[10px] uppercase font-mono text-subtle">
            Sources: {sourceLabel}
          </p>
