  ...
  return (
    <div ref={rootRef} className="relative min-w-0">
      <div className={`flex h-12 items-center rounded-xl border bg-black/20 transition duration-150 ${open ? "border-white/25 bg-white/[.045]" : "border-white/10 hover:border-white/15"}`}>
        ...
        <input value={value} onChange={(event) => onChange(event.target.value)} onFocus={() => { if (suggestions.length) setOpen(true); }} onKeyDown={(event) => { if (event.key === "Escape") { setOpen(false); return; } if (!open || !suggestions.length) { if (event.key === "Enter") onSubmit(); return; } if (event.key === "ArrowDown") { event.preventDefault(); setActiveIndex((index) => (index + 1) % suggestions.length); } else if (event.key === "ArrowUp") { event.preventDefault(); setActiveIndex((index) => (index - 1 + suggestions.length) % suggestions.length); } else if (event.key === "Enter") { event.preventDefault(); select(suggestions[activeIndex]); } }} placeholder="Search jobs by title or role" autoComplete="off" spellCheck={false} aria-label="Search jobs by title or role" aria-autocomplete="list" aria-expanded={open} className="h-full min-w-0 flex-1 bg-transparent px-3 text-sm text-white outline-none placeholder:text-white/25" />
