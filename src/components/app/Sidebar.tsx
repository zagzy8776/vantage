...
const NAV = [
  ...
  { label: "History", href: "/history", badge: "Saved", icon: "M3 12a9 9 0 1018 0 9 9 0 00-18 0zm9-5v5l3 2" },
  ...
  { label: "Intelligence", href: "/intelligence", badge: "AI", icon: "M9 19v-6a2 2 0 00-2-2H5a2 2 0 00-2 2v6a2 2 0 002 2h2a2 2 0 002-2zm0 0V9a2 2 0 012-2h2a2 2 0 012 2v10" },
...
const MOBILE_NAV = [
  ...
  { label: "AI", href: "/intelligence", icon: "M12 3v18M3 12h18M5.6 5.6l12.8 12.8M18.4 5.6L5.6 18.4" },
];

function NavIcon({ path, active = false }: { path: string; active?: boolean }) {
  return <svg className={cn("w-4 h-4 shrink-0", active ? "text-accent" : "text-subtle")} fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.75} d={path} /></svg>;
}
...
export function Sidebar({ isCollapsed, onToggleCollapse, isMobileOpen, onMobileClose, user }: SidebarProps) {
  ...
  return (
    <>
      ...
      <nav aria-label="Mobile navigation" className="fixed bottom-0 left-0 right-0 z-30 lg:hidden border-t border-border bg-surface/95 backdrop-blur px-2 py-2 pb-[calc(0.5rem+env(safe-area-inset-bottom))] grid grid-cols-4 gap-1">
        {MOBILE_NAV.map((item) => { const active = item.href === "/" ? pathname === "/" : pathname.startsWith(item.href); return <Link key={item.href} href={item.href} className={cn("flex flex-col items-center gap-1 rounded-lg py-2 text-[10px] font-medium", active ? "text-accent bg-accent/10" : "text-subtle")}><NavIcon path={item.icon} active={active} /><span>{item.label}</span></Link>; })}
      </nav>
    </>
  );
}
