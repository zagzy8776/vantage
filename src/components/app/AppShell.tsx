  ...
  return (
    <div className="min-h-screen bg-background text-foreground font-sans flex items-center justify-center">
      <div className="flex flex-col items-center gap-3">
        ...
        <span className="text-[10px] font-mono uppercase tracking-[0.25em] text-subtle">
          Checking session
        </span>
      </div>
    </div>
  ...
  return (
    <div className="min-h-screen bg-background text-foreground font-sans flex items-center justify-center p-6">
      <div className="border border-border rounded-lg bg-surface p-6 max-w-sm text-center space-y-3">
        <h1 className="text-sm font-bold text-foreground font-mono uppercase tracking-wide">
          Session unavailable
        </h1>
...
export function AppShell({ children }: AppShellProps) {
  ...
  const applySessionResult = useCallback((result: CurrentUser | null | "error") => {
    if (result === "error") {
      // Server/network failure - never treat this as "not logged in"
      setStatus("error");
    } else if (result) {
      setUser(result);
  ...
  const checkSession = useCallback((): Promise<void> => {
    return fetchCurrentUser()
      ...
      .catch(() => applySessionResult("error"));
  }, [applySessionResult]);
  ...
  useEffect(() => {
    ...
    const suffix =
      nextPath && nextPath !== "/login" ? `?next=${encodeURIComponent(nextPath)}` : "";
