...
function links(html: string, baseUrl: string, job: NormalizedJob) {
  ...
  while ((match = pattern.exec(html)) !== null) {
    ...
    const normalized = normalizeText(`${label} ${href}`);
    score += titleTokens.filter((token) => normalized.includes(token)).length * 2;
...
function jobTextMatch(text: string, job: NormalizedJob) {
  ...
  const exactTitle = lower.includes(job.title.toLowerCase());
  const titleTokens = normalizeText(job.title).split(" ").filter((token) => token.length > 2);
  const titleHits = titleTokens.filter((token) => lower.includes(token)).length;
  return exactTitle || (titleTokens.length > 0 && titleHits >= Math.max(2, Math.ceil(titleTokens.length * 0.6)));
}

function bestEmployerUrl(job: NormalizedJob, url: string) {
  return companyHostMatch(job.companyName, url) ? url : job.companyWebsite;
}
...
export async function researchJobSource(job: NormalizedJob): Promise<NormalizedJob> {
  ...
  const directSource = safeUrl(job.sourceUrl);
  const directApply = safeUrl(job.applyUrl);
  const seedUrls = Array.from(new Set([directSource, directApply, safeUrl(job.companyWebsite)].filter(Boolean) as string[]));

  let best = job;
  let researchedText = "";

  // First inspect URLs already supplied by a provider. This is cheaper and gives us a
  // chance to turn a provider's final/source URL into a real employer/ATS application path.
  for (const seed of seedUrls) {
    if (isBlockedHost(seed)) continue;
    const page = await fetchPublic(seed);
    if (!page) continue;
    const pageText = extractDescription(page);
    const employerMatch = companyHostMatch(job.companyName, seed) || pageText.toLowerCase().includes(job.companyName.toLowerCase());
    const titleMatch = jobTextMatch(pageText, job);
    if (!employerMatch && !titleMatch) continue;
    researchedText = pageText.length > researchedText.length ? pageText : researchedText;
    const candidates = links(page, seed, job);
    const sameEmployerHost = host(seed);
    const application = candidates.find((candidate) => {
      const candidateHost = host(candidate.url) ?? "";
      const sameEmployer = Boolean(sameEmployerHost && (candidateHost === sameEmployerHost || candidateHost.endsWith(`.${sameEmployerHost}`)));
      const ats = ATS_HOSTS.some((ats) => candidateHost === ats || candidateHost.endsWith(`.${ats}`));
      return sameEmployer || ats;
    })?.url;
    best = {
      ...best,
      companyWebsite: bestEmployerUrl(job, seed) ?? best.companyWebsite,
      companyDomain: host(bestEmployerUrl(job, seed)) ?? best.companyDomain,
      sourceUrl: seed,
      applyUrl: application ?? best.applyUrl,
      description: pageText.length > (best.description?.length ?? 0) ? pageText : best.description,
      verificationReasons: [...new Set([...best.verificationReasons, "Public source research matched the named employer and role."])],
    };
    if (application || best.companyWebsite) break;
  }

  // If provider URLs were missing or unusable, resolve the job through public search.
  if (!best.applyUrl || !best.companyWebsite || researchedText.length < 400) {
    ...
    const searches = await Promise.all(queries.slice(0, 3).map((query) => searchEvidence({
    ...
    const items = searches.flatMap((search) => search.results.flatMap((result) => result.results));
    const ranked = items
      .map((item) => ({ item, score: scoreResult(job, item) }))
      .filter((candidate) => candidate.score > 0)
      .sort((a, b) => b.score - a.score)
      .slice(0, 8);

    for (const candidate of ranked) {
      const url = safeUrl(candidate.item.url);
      ...
      const candidateLinks = links(page, url, job);
      const application = candidateLinks.find((link) => {
        const h = host(link.url) ?? "";
        const sourceHost = host(url) ?? "";
        const sameEmployer = h === sourceHost || h.endsWith(`.${sourceHost}`);
      ...
      best = {
        ...
        applyUrl: application ?? best.applyUrl,
        description: pageText.length > (best.description?.length ?? 0) ? pageText : best.description,
        verificationReasons: [...new Set([...best.verificationReasons, "Public web research found a job page matching the named company, role, and location context."])],
      };
...
export async function researchJobs(jobs: NormalizedJob[], maxJobs = Number(process.env.JOB_SOURCE_RESEARCH_LIMIT) || 20) {
  ...
  async function worker() {
    while (queue.length) {
      ...
      if (!job) return;
      try { results.set(job.id, await researchJobSource(job)); }
      catch { results.set(job.id, job); }
    }
  }
