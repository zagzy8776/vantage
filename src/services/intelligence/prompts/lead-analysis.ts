...
export function buildLeadAnalysisUserPrompt(input: LeadIntelligenceInput) {
  return `Analyze this lead using only the evidence below. Missing fields are UNKNOWN, not negative evidence. Every FACT, DERIVED, and INFERENCE evidence claim must cite the exact supplied evidence IDs in evidenceIds. Decide whether the opportunity is worth pursuing without assuming a project is needed.\n\nSTRUCTURED EVIDENCE:\n${JSON.stringify(input, null, 2)}`;
}

export function buildRepairPrompt(content: string) {
  return `The previous response was not valid for the required VANTAGE JSON schema. Repair it using only its existing supported claims. Return only a valid JSON object with the required keys and valid enum/range values. Do not add facts. Previous response:\n${content}`;
}